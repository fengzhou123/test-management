...
//    public static void main(String[] args) {}
    ...
    public ERROR_CODE runSubTest(String hostname, String username, String password, int testIndex) {
        ...
        String espAvailability = TestcaseConfigHelper.getTestcaseSubTestConfigValue(testConfig, testIndex, "espavailability");
        String espDBName = (espAvailability.equalsIgnoreCase("true")) ?
                TestcaseConfigHelper.getTestcaseSubTestConfigValue(testConfig, testIndex, "espdbname") : "noESPDBName";
        String espBackupFilePath = (espAvailability.equalsIgnoreCase("true")) ?
                TestcaseConfigHelper.getTestcaseSubTestConfigValue(testConfig, testIndex, "espbackupfilepath") : "noEspBackupFilePath";
...
/*
        ...
        if (espAvailability.equalsIgnoreCase("true")) {
...
*/
        ...
        if (KDWHelper.installOrUpgradeKDW(hostname, username, password, kdwBaseDirectory, kdwRev2InstallerDirectory,
                ...
                kdwRev2Build, KDWHelper.kdwUpgradeCode) == false) {
        ...
        // Copy MYSQL connector, kp_sites_n1_n.xml and kp_etl.xml files to the remote machine
        logger.severe("Step6-1: Copy MYSQL connector and kp_sites_ip.xml files to the remote machine");
...
/*
        ...
        if (KDWHelper.invokeDwdateInitAndLoadScheduleConfigure(hostname, username, password, kdwUpgradeSiteIdentifier, sourceDBType) == false) {
...
/*
        // check result
        logger.severe("Step7: Install and configure revision2 KDW, run ETL and validate results");
        if (InstallAndConfigureKdwRunEtlAndCheckResults(hostname, username, password, kdwDBType, kdwDBHostname, kdwDBPort, kdwDBUsername,
                kdwDBPassword, kdwDBPasswordEncrypted, kdwBaseDirectory, kdwRev2InstallerDirectory,
                kdwRev2SiteIdentifier, installerJenkinsHostname, installerJenkinsUsername, installerJenkinsPassword,
                rev2AppJavaHome, kdwRevision2, kdwRev2Build, kpSitesFile, mySqlConnectorFile, period) == false) {
 ...
 */
    ...
    public boolean InstallAndConfigureKdwRunEtlAndCheckResults(String hostname, String username, String password, String kdwDBType,
        ...
        if (KDWHelper.installOrUpgradeKDW(hostname, username, password, kdwBaseDirectory, kdwInstallerDirectory, kdwSiteIdentifier,
            ...
            logger.severe("Failed to Install KDW revision : " + kdwRevision + kdwBuild);

            return false;
        }

        // Copy MYSQL connector, kp_sites_n1_n.xml and kp_etl.xml files to the remote machine
        logger.severe("Copy MYSQL connector and kp_sites_n1_n.xml files to the remote machine");
